			...
			if ( !empty( $td_options_fb_connected_account ) ) {
				...
				if ( isset( $td_options_fb_connected_account['fb_account_pages_data'] ) && is_array( $td_options_fb_connected_account['fb_account_pages_data'] ) ) {
					...
					if ( empty( $fb_account_pages_data_ig_account_remove_status ) ) {
						$reply['status'] = 'error - no instagram business account found with the id: ' . $_POST['account_id'];
					} else {
				...
				if ( isset( $td_options_fb_connected_account['fb_account_pages_data'] ) && is_array( $td_options_fb_connected_account['fb_account_pages_data'] ) ) {
					...
					foreach ( $td_options_fb_connected_account['fb_account_pages_data'] as $index => $page_data ) {
						...
						if ( $_POST['account_id'] === $page_data['instagram_business_account']['id'] ) {
							...
							// set reply status
							$fb_account_pages_data_ig_account_remove_status = 'the instagram ' . $_POST['account_username'] . ' business account was deleted from fb connected account data';
					...
					if ( empty( $fb_account_pages_data_ig_account_remove_status ) ) {
						$reply['fb_account_pages_data_status'] = 'no instagram business account found with the id: ' . $_POST['account_id'] . ' in fb connected account pages data.';
					} else {
			...
			if ( !empty( $td_instagram_business_accounts ) && is_array( $td_instagram_business_accounts ) ) {
				foreach ( $td_instagram_business_accounts as $key => $account ) {
					if ( $_POST['account_id'] === $account['id'] ) {
						...
						// set reply status
						$ig_business_accounts_ig_account_remove_status = 'success - the instagram ' . $_POST['account_username'] . ' business account and associated cached data deleted';
