...
//        var_dump($result);
	...
	static function on_ajax_subscription_register() {
        ...
        if ( defined( 'TD_SUBSCRIPTION' ) && version_compare(TD_SUBSCRIPTION_VERSION, '1.5.2', '>=') ) {
            ...
            /* -- Retrieve all free plans which have the auto subscribe setting enabled. -- */
            global $wpdb;

            $plans = $wpdb->get_results("SELECT * FROM tds_plans WHERE is_free = 1 AND auto_subscribe = 1", ARRAY_A);
